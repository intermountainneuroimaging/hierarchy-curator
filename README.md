...
You can save information from any level into the class instance attribute `data` itself, and then access that lower down.  For example if you wanted to set file level information based on subject metadata, you could write a curator like the following:
...
3. The inputs must be accessed with the special `self.open_input` context manager (See below).
